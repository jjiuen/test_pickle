...
pickle = importlib.machinery.SourceFileLoader('pickle', './std_pickle2/pickle2.py').load_module()
...
class SimpleClass(object):
    ...
    def __repr__(self):
        return "SimpleClass({}, {})".format(self.x, self.y)
...
class ReduceClass(object):
    ...
    def __repr__(self):
        return "ReduceClass({!r})".format(self.name)
...
class SlotClass(object):
    ...
    def __repr__(self):
        return "SlotClass({}, {})".format(self.a, self.b)
...
class ExternalObject(object):
    ...
    def __repr__(self):
        return "ExternalObject({!r})".format(self.external_id)
...
def test_pickle_coverage():
    ...
    for obj in test_objects:
        print("\nTesting object: {0}".format(repr(obj)))
        for proto in range(pickle.HIGHEST_PROTOCOL + 1):
            print("  Protocol {} ...".format(proto), end=" ")
            buf = io.BytesIO()
            pickler = MyPickler(buf, protocol=proto)
            try:
                pickler.dump(obj)
            except Exception as e:
                print("dump failed: {}".format(e))
                continue
            ...
            assert restored == obj, "Mismatch: {} != {}".format(restored, obj)

    for obj in recursive_objects:
        print("\nTesting recursive object: {0}".format(repr(obj)))
        for proto in range(pickle.HIGHEST_PROTOCOL + 1):
            print("  Protocol {} ...".format(proto), end=" ")
            try:
                ...
                print("Recursive test failed: {}".format(e))
    ...
    try:
        ...
        print("Expected exception caught: {}".format(e))


def main():
    cov = coverage.Coverage(source=["std_pickle2"])
    cov.start()

    test_pickle_coverage()

    tester = PurePythonPickleTester(pickle_path="./std_pickle2/pickle2.py")
